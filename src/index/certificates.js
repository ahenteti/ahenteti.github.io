...
const CERTIFICATES_OF_COMPLETION = [
  'assets/img/certificate-advanced-defensive-programming-techniques.png',
...
$(window).on(constants.PAGE_CONTENT_READY_EVENT, () => {
  const certificatesOfCompletionArrowBack = document.querySelector('.certificate-of-completion-image-container [name="caret-back"]');

  const certificatesOfCompletionArrowForward = document.querySelector('.certificate-of-completion-image-container [name=caret-forward]');
  ...
  certificatesOfCompletionArrowForward.addEventListener('click', () => handleCertificatesOfCompletionNavigation(FORWARD_ARROW));
});
