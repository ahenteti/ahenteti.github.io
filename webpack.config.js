...
const entries = glob.sync('./src/{index,about,articles}/**/*.js').reduce((acc, filePath) => {
...
const htmlWebpackPlugins = glob.sync('./src/{index,about,articles}/**/*.html').reduce((acc, filePath) => {
...
function getArticlesMetadata () {
  const articles = glob.sync('./src/articles/**/metadata.json').reduce((articles, filePath) => {
...
function getArticlesTags () {
  const tags = glob.sync('./src/articles/**/metadata.json').reduce((tags, filePath) => {
    JSON.parse(fs.readFileSync(filePath)).tags.split(',').forEach(tag => tags.add(tag));
    return tags;
  }, new Set());
